...
class Modelo{
    ...
    public function abrirCaja($nombre, $fecha){
        $query = $this->db->prepare("INSERT INTO cajadiaria (inicioSesion, egresoSesion, empleado, totalCosto, totalVenta, limpio, cantProductos, cantClientes) VALUES (?,?,?,?,?,?,?,?)");
        $query->execute([$fecha, 0, $nombre, 0, 0, 0, 0, 0]);
    } 
